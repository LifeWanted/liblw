...
TEST(TLS, CanConstructFactory) {
  TLSStreamFactory factory{
      {.private_key = testing::KEY_PATH, .certificate = testing::CERT_PATH}};
}
...
protected:
  TLSStreamTest()
      : _client_factory{{.private_key = testing::KEY_PATH, .certificate = testing::CERT_PATH, .connection_mode = TLSOptions::CONNECT}},
        _server_factory{
  ...
  ~TLSStreamTest() { co::testing::destroy_all_schedulers(); }
...
TEST_F(TLSStreamTest, HandshakesFinish) {
  scheduler().schedule([&]() -> co::Task { co_await _client->handshake(); });
  scheduler().schedule([&]() -> co::Task { co_await _server->handshake(); });
  scheduler().run();
}
