...
public:
    ...
    /// @brief  Create a buffer wrapped around externally allocated memory,
    ///         optionally taking ownership of the memory.
    ///
    /// @param buffer   A pointer to the beginning of the buffer.
    /// @param capacity The size of the buffer in bytes.
    /// @param ownData  Flag indicating if this `Buffer` should take ownership
    ///                 of the memory.
    ...
    ///
    /// The input iterator must support `operator-( const InputIterator&, const
    /// InputIterator&)` as a way of determining the distance between two
    /// iterators. On top of that it must support dereferencing (`operator*()`)
    /// and prefix increment (`operator++()`).
    ///
    /// @tparam InputIterator   A generic forward-iterator whose dereferenced
    ///                         value must be convertible to `byte`.
    ///
